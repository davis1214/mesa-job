...
object UrlCostsStatOnline {
  ...
  def main(args: scala.Array[String]) {
    ...
    val parsedRdd = sourceRdd.map(line => {
      ...
      val span = if (f._3 <= 50) "50ms"
      else if (f._3 > 50 && f._3 <= 100) "100ms"
      else if (f._3 > 100 && f._3 <= 200) "200ms"
      else if (f._3 > 200 && f._3 <= 500) "500ms"
      else "1s"

      val key1 = f._2.concat("@@").concat(f._1).concat("@@").concat(span)
      (key1, 1)
    }).reduceByKey(_ + _, 10)
